                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            @foreach($departments as $dept)
                                                <li>
                                                    <a class="dropdown-item" href="#" onclick="showUploadModal('department', {{ $dept->id }}, '{{ $dept->name }}')">
                                                        {{ $dept->name }}
                                                    </a>
                                                </li>
                                            @endforeach
                            ...
                            <div class="card text-center h-100 upload-option" data-scope="study_program">
                                <div class="card-body">
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            @foreach($departments as $dept)
                                                <li><h6 class="dropdown-header">{{ $dept->name }}</h6></li>
                                                @foreach($dept->studyPrograms as $program)
                                                    <li>
                                                        <a class="dropdown-item" href="#" onclick="showUploadModal('study_program', {{ $program->id }}, '{{ $program->name }}')">
                                                            {{ $program->name }}
                                                        </a>
                                                    </li>
                                                @endforeach
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu" style="max-height: 300px; overflow-y: auto;">
                                            @foreach($departments as $dept)
                                                <li><h6 class="dropdown-header">{{ $dept->name }}</h6></li>
                                                @foreach($dept->studyPrograms as $program)
                                                    <li><small class="dropdown-header text-muted">{{ $program->name }}</small></li>
                                                    @foreach($program->classes as $class)
                                                        <li>
                                                            <a class="dropdown-item" href="#" onclick="showUploadModal('class', {{ $class->id }}, '{{ $class->fullName }}')">
                                                                {{ $class->name }}
                                                            </a>
                                                        </li>
                                                    @endforeach
                                    ...
                                    <tbody>
                                        @forelse($recentUploads as $upload)
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-sm btn-outline-primary" onclick="viewBatchDetails('{{ $upload['batch_id'] }}')">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="uploadForm" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Upload untuk: <span id="scopeLabel" class="fw-bold text-primary"></span></label>
                    </div>
                    
                    <div class="mb-3">
                        <label for="data_file" class="form-label">File Data</label>
                        <input type="file" class="form-control" id="data_file" name="data_file" accept=".csv,.xlsx,.xls" required>
                        <div class="form-text">
                            <strong>Format yang didukung:</strong> CSV, Excel (.xlsx, .xls)<br>
                            <strong>Struktur Excel:</strong> NO, Nama, Jur/Kls/Prodi, Listening, Structure, Reading, Total<br>
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="uploadBtn" onclick="uploadFile()">Upload & Preview</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="previewModal" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-success" id="processBtn" onclick="processData()">Process Data</button>
            </div>
        </div>
    </div>
</div>
